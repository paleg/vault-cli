...
@pytest.fixture
def mock_hvac_v2_class(mocker):
    client = mocker.patch("hvac.Client")
    subclient=client.return_value
    mocker.patch.object(subclient, "read", return_value={"data":{"options": {"version":"2"}}})
    mocker.patch.object(subclient.secrets.kv.v2, "create_or_update_secret", return_value=None)
    mocker.patch.object(subclient.secrets.kv.v2, "read_secret", return_value=None)
    mocker.patch.object(subclient.secrets.kv.v2, "delete_metadata_and_all_versions", return_value=None)
    mocker.patch.object(subclient.secrets.kv.v2, "list_secrets", return_value=None)
    yield client
    #yield mocker.patch("hvac.Client", lambda *_, **__:client)

@pytest.fixture
def mock_hvac_v2(mock_hvac_v2_class):
    #yield mock_hvac_v2_class
    yield mock_hvac_v2_class.return_value


def test_token(mock_hvac_v2):
    get_client()
    assert mock_hvac_v2.token == "tok"


def test_userpass(mock_hvac_v2):

    get_client(token=None, username="myuser", password="pass")

    mock_hvac_v2.auth_userpass.assert_called_with("myuser", "pass")


def test_certificate(mock_hvac_v2_class, mock_hvac_v2):

    get_client(token=None, login_cert="a", login_cert_key="b")

    assert mock_hvac_v2_class.call_args[1]["cert"] == ("a", "b")
    mock_hvac_v2.auth_tls.assert_called_with()
...
def test_get_secret(mock_hvac_v2):
    ...
    mock_hvac_v2.secrets.kv.v2.read_secret.assert_called_with("a", mount_point="bla")


def test_get_secret_not_found(mock_hvac_v2):

    mock_hvac_v2.secrets.kv.v2.read_secret.return_value = None
...
def test_list_secrets(mock_hvac_v2):
    mock_hvac_v2.secrets.kv.v2.list_secrets.return_value = {"data": {"keys": ["b"]}}

    assert get_client()._list_secrets("bla/a") == ["b"]

    mock_hvac_v2.secrets.kv.v2.list_secrets.assert_called_with("a", mount_point="bla")


def test_list_secrets_sorted(mock_hvac_v2):
    mock_hvac_v2.secrets.kv.v2.list_secrets.return_value = {"data": {"keys": ["b", "A", "c"]}}

    assert get_client()._list_secrets("bla/a") == ["A", "b", "c"]


def test_list_secrets_empty(mock_hvac_v2):
    mock_hvac_v2.secrets.kv.v2.list_secrets.return_value = None
...
def test_delete_secret(mock_hvac_v2):

    get_client().delete_secret("a")

    mock_hvac_v2.secrets.kv.v2.delete_metadata_and_all_versions.assert_called_with("a", mount_point="bla")
...
def test_delete_secret_many_keys(mock_hvac_v2):
    ...
    mock_hvac_v2.secrets.kv.v2.delete_metadata_and_all_versions.assert_not_called()
...
def test_delete_secret_missing_key_or_mapping(mock_hvac_v2, existing_mapping):
    ...
    mock_hvac_v2.secrets.kv.v2.delete_metadata_and_all_versions.assert_not_called()
    mock_hvac_v2.secrets.kv.v2.create_or_update_secret.assert_not_called()


def test_set_secret(mock_hvac_v2):

    get_client()._set_secret("bla/a", {"value": "b"})

    mock_hvac_v2.secrets.kv.v2.create_or_update_secret.assert_called_with("a", secret={"value": "b"}, mount_point="bla")


def test_lookup_token(mock_hvac_v2):

    get_client()._lookup_token()

    mock_hvac_v2.lookup_token.assert_called_with()
